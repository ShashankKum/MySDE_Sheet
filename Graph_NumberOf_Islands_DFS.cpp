//we can have a single set with values adjacent such that it is either up down left or right, so we recur in all these directions keeping base case in mind.
//It is solved just as dfs for multiple directions.
//In dfs also we have mark and recur.
